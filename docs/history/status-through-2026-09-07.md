# 2026-09-07までの作業記録（旧STATUS原文）

2026-09-07に現在地と履歴を分離した際の保存記録。以下の原文は並び順・見出し・記述をそのまま保持している。「現在」「アクティブ」「残り」「100%」は各記録時点・各作業範囲の表現であり、最新状態ではない。冒頭の更新日と後続報告の日付の不一致も原文のまま残している。

現在地は[STATUS.md](../../STATUS.md)、現在の作業目標は[GOAL.md](../../GOAL.md)を参照する。
...
## アヤ凍傷編成横断反映 実装前診断スライス7の終了報告

- 達成度: 100%（診断スライス）。A2・愛用品アヤを編成した別の冷静使徒で、タイムラインとDPS数値が変化しない原因を特定した。計算コードは変更していない。
- 根拠: `formation-damage-calc.js`の`createDpsFormationStatusReactions()`は、別の冷静使徒へ凍傷reaction定義を1件登録するだけで、凍傷スタックを生成しない。`createDpsFormationEventCandidates()`は選択中使徒の`actionDamageData.audit`だけを走査するため、編成内アヤのA2・愛用品の状態付与は候補化されない。`getDpsEffectiveExternalEvents()`にも編成アヤの状態providerは存在しない。
- 不足要素: (1)編成アヤのA2・愛用品・高学年を状態専用候補へ収集する処理、(2)低学年SP／愛用品10秒／高学年個別OFFの発生時刻provider、(3)実効イベントへの凍傷スタック付与、(4)1イベント複数スタックを扱う`statusStackCount`、(5)タイムライン・DPS数値の編成横断テストが未実装。現在のテストはreactionの登録と、選択中アヤ本人の凍傷経路だけを確認している。
- 判定: 凍傷reactionは`state.statusStacks`に凍傷が存在するときだけ`+8%／スタック`を返すため、編成アヤからスタックが供給されない現状では別使徒のDPSは変化しない。今回の画面確認結果は現行実装どおりであり、A2・愛用品の有効判定が失敗したことを示すものではない。
- 残り見積もり: 実装約3スライス。編成状態providerとスタック数、シミュレーター接続、設定UI・二重発火防止・数値／タイムライン回帰の順に実装する。高学年初期OFFとdatasheet無変更は維持する。

## アヤ凍傷軽量接続 設計スライス8の終了報告

- 達成度: 100%（設計スライス）。アヤの低学年ヒットをフレーム単位で再現せず、別の冷静使徒へ凍傷ダメージ補正だけを渡す軽量接続方式を確定した。計算コードは変更していない。
- 根拠: 編成アヤのcombatantは作らず、A2・高学年・愛用品の状態専用candidateを最大3件だけ生成する。低学年1回を敵サイズ別2／4／5／8スタックの単一イベントにまとめ、内部展開も上限9回とする。推定時刻はsnapshot作成時に一度生成し、試行ごとは既存イベントを再生するため、通常の行動・命中シミュレーション増加を避けられる。
- 計算境界: `statusReactionOnly`を追加し、編成アヤ由来の凍傷スタックは冷静の`+8%／スタック`判定だけへ使用する。アヤの凍傷DoT、直接ダメージ、モーション、SP、選択中使徒の状態付与トリガーは発生させない。共通`凍傷:stack:9`、持続600F、上限9、個別期限は既存状態処理を使う。
- 発動方針: A2は低学年SP周期の`自動（推定）`、愛用品は10秒周期の`自動`、高学年は個別bindingの初期OFF。低学年は初回SP充填時間と2回目以降の必要SP周期を分け、敵サイズ不明時は1スタックへ保守的にフォールバックする。手動周期・発生秒・スタック数は同じbindingの自動providerを置換する。
- 表示方針: 同一フレームの一括付与はタイムラインへ1行で表示し、内部の各スタックをヒット別に並べない。設定カードでは由来、1回の付与数、持続10秒、自動／推定／OFFを表示する。
- 残り見積もり: 約3スライス。(1)候補収集と軽量provider、(2)`statusStackCount`／`statusReactionOnly`のシミュレーター接続、(3)設定UI・保存・本体／試験版・数値／タイムライン回帰。datasheetは変更しない。

## アヤ凍傷軽量接続 実装スライス9の終了報告

- 達成度: 約85%。編成中アヤのA2・愛用品・高学年を、選択中の別の冷静使徒へ凍傷状態だけ投影し、冷静被ダメージ補正へ接続した。アヤ本人の行動・ダメージ・SP・DoTは複製しない。
- 根拠: `formation-damage-calc.js`で敵サイズ別のA2推定スタック数とSP周期、愛用品10秒周期、高学年個別候補を生成し、`dps-trigger-policy.js`と本体／試験版UIで同じstatus metadataを保持する。`dps-simulator.js`は`statusStackCount`を一括適用し、タイムラインを集約表示する。`statusReactionOnly`により凍傷DoT、状態付与連鎖、スタック最大連鎖を抑止し、既存の凍傷reactionだけを利用する。
