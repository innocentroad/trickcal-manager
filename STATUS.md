...
- source対象限定commitは `e23ee0024c9872a48d43b2c91c7281748ce24042`。candidateは `857395a26ad71669`、contentDigestは `0127658afed59e1614fed8ec1536e5ab11944ce55eff908ab58c3a3c9fdbb7d6`。新サイトartifactは `665c69787c4477436fbbd8b4173dcb62a5f57305`、runは `https://github.com/innocentroad/trickcal-manager-site/actions/runs/35360331718`、通常deployment承認後にpublished。新identityは `https://trickcal.irlab.dev/public-site-deployment.json` でcandidate/source/profile/digest一致を確認した。
- 旧サイトartifactは `c7ce42f847b77fe473888276d06e690d6d0d1856`、runは `https://github.com/innocentroad/trickcal-manager/actions/runs/35360718102`、新サイト成功後に通常deployment承認を経てpublished。旧identityは `https://innocentroad.github.io/trickcal-manager/public-site-deployment.json` で同じcandidate/source/contentDigest、profile=legacy、outputDigest `d7d75afc5d24f9cb` を確認した。新outputDigestは `e137f2acf5ac30b1`。
- bundleは `tmp/publication-20260918150244174.json`、new receiptは `tmp/delivery-857395a26ad71669-new-f8e9364888cb.json`、legacy receiptは `tmp/delivery-857395a26ad71669-legacy-d10d9eaf205f.json`。公開記録は `backups/publication-history/857395a26ad71669-new.json` と `...-legacy.json`。
- 実ブラウザで新旧のmanager/calc/data/enemy/board入口、共通上バーの現在位置、使徒データ78行と画像79件（broken 0）、共有payloadの画像379件（broken 0、`.webp%3Fv%3D` 0）を確認した。隔離native検査では720px以下の2段、使徒データ5ビュー、低い画面の絞り込み展開、装備詳細、通知連携を確認した。
- 共有ページ先頭の既存文字列 `\\n` は今回の公開範囲外（shared page newline fix）であり残っている。共有画像URL修正は保持され、画像ロードには影響していない。公開ブラウザではベルを確認したが、移行バーの実表示は既存ブラウザの通知状態に依存するため、利用者状態を変更せず確認対象外とした。隔離fixtureで表示条件・連携は確認済み。
- 残る既存阻害は `test-formation-share-maintenance.js` の `tools/git/push.bat` 固定前提不一致と、`test-public-site-staging.js` の旧固定identity/digest前提。今回の公開成功へ混入させず、範囲外修正もしていない。mainの未公開案件は引き続き別管理とする。
