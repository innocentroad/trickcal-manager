...
assert.ok(main.includes('formation-damage-dps-prototype.js?v=20260903a'), '本体は共有DPS controllerの新cache-bustを参照する');
assert.ok(dpsPage.includes('app-cache.js?v=20260903c') && dashboard.includes('app-cache.js?v=20260903c'), 'DPS試験版・ステータス管理も最新app-cacheを参照する');
...
assert.ok(main.indexOf('formation-damage-calc.js?v=20260903c') < main.indexOf('formation-damage-dps-prototype.js?v=20260903a'), '共有DPS controllerは通常計算snapshot APIの後に読む');
...
assert.match(cache, /calc: \[[\s\S]*formation-damage-dps-prototype\.css\?v=20260902c[\s\S]*dps-timing-data\.js\?v=20260903a[\s\S]*dps-simulator\.js\?v=20260902d[\s\S]*dps-support-registry\.js\?v=20260827c[\s\S]*formation-damage-dps-prototype\.js\?v=20260903a[\s\S]*\]/, '通常計算routeのcache warmupへDPS依存を加える');
