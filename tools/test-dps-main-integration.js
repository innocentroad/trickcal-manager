...
[
  ...
  'fdcp-dps-compare-slot', 'fdcp-dps-settings-slot', 'fdcp-dps-run', 'fdcp-baseline-save', 'fdcp-baseline-clear', 'fdcp-high-mode-quick', 'fdcp-dps-external-control', 'fdcp-dps-external-event-count'
...
assert.ok(main.includes('formation-damage-dps-prototype.css?v=20260828t'), '本体は共有DPS stylesheetの新cache-bustを参照する');
assert.ok(main.includes('formation-damage-dps-prototype.js?v=20260828k'), '本体は共有DPS controllerの新cache-bustを参照する');
...
assert.ok(main.indexOf('formation-damage-calc.js?v=20260828b') < main.indexOf('formation-damage-dps-prototype.js?v=20260828k'), '共有DPS controllerは通常計算snapshot APIの後に読む');

assert.match(cache, /calc: \[[\s\S]*formation-damage-dps-prototype\.css\?v=20260828t[\s\S]*dps-timing-data\.js\?v=20260827b[\s\S]*dps-simulator\.js\?v=20260827n[\s\S]*dps-support-registry\.js\?v=20260827c[\s\S]*formation-damage-dps-prototype\.js\?v=20260828k[\s\S]*\]/, '通常計算routeのcache warmupへDPS依存を加える');
