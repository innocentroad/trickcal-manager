...
assert.ok(main.includes('formation-damage-dps-prototype.css?v=20260903d'), '本体は共有DPS stylesheetのcache-bustを維持する');
...
assert.ok(main.includes('formation-damage-dps-prototype.js?v=20260903e'), '本体は共有DPS controllerの新cache-bustを参照する');
assert.ok(dpsPage.includes('app-cache.js?v=20260903h') && dashboard.includes('app-cache.js?v=20260903h'), 'DPS試験版・ステータス管理は最新app-cacheを参照する');
assert.ok(dashboard.includes('dps-trigger-policy.js?v=20260903e'), 'ステータス管理のpolicy prefetchも最新cache-bustを参照する');
assert.ok(dpsPage.includes('formation-dps-calc.js?v=20260903e'), 'DPS試験版controllerの新cache-bustを参照する');
...
assert.ok(main.indexOf('dps-timing-data.js?v=20260903a') < main.indexOf('formation-damage-calc.js?v=20260903g'), 'timing dataは本体controllerより先に読む');
assert.ok(main.indexOf('dps-simulator.js?v=20260902d') < main.indexOf('formation-damage-calc.js?v=20260903g'), 'DPS simulatorは本体controllerより先に読む');
assert.ok(main.indexOf('dps-support-registry.js?v=20260827c') < main.indexOf('formation-damage-calc.js?v=20260903g'), '対応registryは本体controllerより先に読む');
assert.ok(main.indexOf('formation-damage-calc.js?v=20260903g') < main.indexOf('formation-damage-dps-prototype.js?v=20260903e'), '共有DPS controllerは通常計算snapshot APIの後に読む');
...
assert.ok(dpsPageScript.includes('getDpsFormationBindingModes') && dpsPageScript.includes('bindingModes: getDpsFormationBindingModes(runtimeEffects)'), 'DPS試験版も保存済みbinding単位の自動・OFF設定を推定providerへ渡す');
...
assert.match(cache, /calc: \[[\s\S]*formation-damage-dps-prototype\.css\?v=20260903d[\s\S]*dps-trigger-policy\.js\?v=20260903e[\s\S]*dps-timing-data\.js\?v=20260903a[\s\S]*dps-simulator\.js\?v=20260902d[\s\S]*dps-support-registry\.js\?v=20260827c[\s\S]*formation-damage-dps-prototype\.js\?v=20260903e[\s\S]*\]/, '通常計算routeのcache warmupへDPS依存を加える');
assert.ok(cache.includes('formation-damage-calc.js?v=20260903g'), 'cache manifestも共有controllerのcache-bustを参照する');
assert.match(cache, /dpsPrototype: \[[\s\S]*formation-damage-dps-prototype\.js\?v=20260903e[\s\S]*dps-trigger-policy\.js\?v=20260903e/, 'prototype routeも共有controllerとpolicyのcache-bustを揃える');
